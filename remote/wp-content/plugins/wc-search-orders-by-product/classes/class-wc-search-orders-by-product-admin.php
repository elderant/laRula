                ...
                if(!empty( $_GET['product_id'] ) && empty($_GET['search_product_type']) && empty($_GET['search_product_cat'])) {
                    ...
                    $order_itemmeta = $wpdb->prefix . "woocommerce_order_itemmeta";
										...
										if(strcasecmp($product -> get_type(), 'variation') == 0) {
											$where .= " AND $product_id IN (SELECT $order_itemmeta.meta_value FROM $order_items LEFT JOIN $order_itemmeta on $order_itemmeta.order_item_id=$order_items.order_item_id WHERE $order_items.order_item_type='line_item' AND $order_itemmeta.meta_key='_variation_id' AND $orders.ID=$order_items.order_id)";
										}
										else {
